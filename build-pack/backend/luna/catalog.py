...
VALID_PROVIDERS = frozenset({"openrouter", "groq", "together", "ollama", "google"})
...
def build_auto_catalog() -> list[LunaModelEntry]:
    ...
    if os.getenv("GOOGLE_API_KEY", "").strip() and google_model:
        out.append(
            LunaModelEntry(f"google|{google_model}", "google", google_model, f"Google · {google_model}")
        )
