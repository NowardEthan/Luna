...
from .openrouter_reasoning import apply_openrouter_request_extras
...
async def openrouter_chat(raw: dict) -> dict:
    ...
    if err:
        return {"ok": False, "error": err}
    apply_openrouter_request_extras(raw, body, model)
...
async def google_chat(raw: dict) -> dict:
    key = os.getenv("GOOGLE_API_KEY", "").strip()
    if not key:
        return {"ok": False, "error": "GOOGLE_API_KEY não configurada."}
    model = str(raw.get("llm_model") or os.getenv("GOOGLE_MODEL", "gemini-2.5-pro"))
    body, err = build_messages_body(raw, model)
    if err:
        return {"ok": False, "error": err}
    return await chat_once(
        ...
        {"Authorization": f"Bearer {key}", "Content-Type": "application/json"},
        body,
        chat_timeout_ms("OPENROUTER_CHAT_TIMEOUT_MS"),
    )


async def google_chat_stream(raw: dict, emit: EmitFn) -> dict:
    key = os.getenv("GOOGLE_API_KEY", "").strip()
    if not key:
        return {"ok": False, "error": "GOOGLE_API_KEY não configurada."}
    model = str(raw.get("llm_model") or os.getenv("GOOGLE_MODEL", "gemini-2.5-pro"))
